...
def process_data(
    dg: pd.DataFrame, test_df: pd.DataFrame, train_df: pd.DataFrame
) -> pd.DataFrame:
...
def load_dataset(
    ...
    if input_test_file:
        ...
        test_df["source_text"] = test_df["dialogue"]
    else:
    ...
    my_dataset_dict = DatasetDict(
        {"train": train_dataset, "test": test_dataset, "validation": valid_dataset}
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="Path to a val csv file into model",
    )
    parser.add_argument(
        "--output_file", type=str, default="system.txt", help="Generate sample"
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    val = not args.no_val
    assert (
        args.input_test_file or args.input_val_file
    ), "Do you want to train only? Really?"
    ...
    for seed in args.seeds:
        ...
        if args.test:
            for index in rn.sample(range(len(test_dataset)), 1):
                logger.info(
                    f"Sample {index} of the testing set: {test_dataset[index]}."
                )
        ...
        if args.train:
            ...
            if args.with_tracking:
                ...
                accelerator.init_trackers("PULSAR", experiment_config)
            ...
            if val:
                ...
                with open(
                    os.path.join(output_dir, f"predictions_{epoch}.json"), "w"
                ) as pf:
                    for p in [
                        {"prediction": k, "label": v}
                        for k, v in zip(epoch_predictions, epoch_labels)
                    ]:
