...
HEADERS = {
    "PLAN": "Conversation about the patient's treatment plan:",
...
def load_dataset(
    ...
    if input_dg_file:
        ...
        dg["source_text"] = (dg["section_header"] + "\n" if header_input else "") + dg[input_field]
        dg["target_text"] = (dg["section_header"] + "\n" if header_output else "") + dg[output_field]
    else:
    ...
    if input_val_file:
        ...
        val_df["source_text"] = (val_df["section_header"] + "\n" if header_input else "") + val_df[input_field]
        val_df["target_text"] = (val_df["section_header"] + "\n" if header_output else "") + val_df[output_field]
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_key", type=str, help="The key of the input field.", default='dialogue')
    parser.add_argument("--output_key", type=str, help="The key of the output field.", default='dialogue')
    parser.add_argument(
        "--header_input", action="store_true", help="whether to use the section header as part of input."
    )
    parser.add_argument(
        "--header_output", action="store_true", help="whether to use the section header as part of output."
    )
    parser.add_argument(
        "--convert_header", action="store_true", help="whether to convert the section header into natural language."
    )
    parser.add_argument("--prefix", type=str, default="summarize: ", help="prefix for flan-T5")
    ...
    for seed in args.seeds:
        ...
        data_collator = DataCollatorForSeq2Seq(
            tokenizer, model=model, pad_to_multiple_of=8 if accelerator.use_fp16 else None
        )
                ...
                for e_step, e_batch in enumerate(eval_dataloader):
                    with torch.no_grad():
                        ...
                        labels = accelerator.pad_across_processes(
                            e_batch["labels"], dim=1, pad_index=tokenizer.pad_token_id
                        )
