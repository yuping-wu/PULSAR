...
"""
Fine-tuning a 🤗 Transformers model for sequence to sequence masked language modeling on a text file or a dataset.
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Continue pretraining T5 model on clinical corpus.")
    parser.add_argument(
        "--dataset_name",
        type=str,
        default=None,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Name of the run to use for tracking.",
    )
    parser.add_argument(
        "--wandb_api_key",
        type=str,
        default=None,
        help="The Wandb API key to use for tracking.",
    )
...
def main():
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        ...
        # Create an instance of the Config class from the loaded data
    ...
    send_example_telemetry("run_t5_no_trainer", args)
    ...
    if args.dataset_name is not None:
        ...
        if os.path.isfile(args.train_file):
            ...
            extension = os.listdir(args.train_file)[0].split(".")[-1]
            data_files["train"] = [os.path.join(args.train_file, f) for f in os.listdir(args.train_file)]
            data_files['validation'] = [os.path.join(args.validation_file, f) for f in os.listdir(args.validation_file)]
    ...
    if args.tokenizer_name:
        logger.info(f"Using tokenizer from {args.tokenizer_name}")
        tokenizer = AutoTokenizer.from_pretrained(args.tokenizer_name, use_fast=not args.use_slow_tokenizer)
    elif args.model_name_or_path:
        logger.info(f"Using tokenizer from {args.model_name_or_path}")
    ...
    dataset_columns = corpus_name_mapping.get(args.dataset_name, None)
    ...
    dummy_inputs = tokenizer(
         ...
         max_length=args.max_source_length, padding=padding, truncation=True, 
         return_tensors='pt'
    )
    ...
    # Prepare everything with our `accelerator` except model.
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        with open(os.path.join(output_dir, "predictions.json"), "w") as pf:
