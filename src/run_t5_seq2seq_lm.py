...
"""
Fine-tuning the library models for sequence to sequence masked language modeling on a text file or a dataset.
...
class DataTrainingArguments:
    ...
    lang: Optional[str] = field(default=None, metadata={"help": "Language id for Multilingual Seq2Seq LM."})
    ...
    source_column: Optional[str] = field(
        default=None,
        metadata={"help": "The name of the column in the datasets containing the full source texts (for Seq2Seq LM)."},
    )
    target_column: Optional[str] = field(
        default=None,
        metadata={"help": "The name of the column in the datasets containing the target text (for Seq2Seq LM)."},
    )
    ...
    mlm_probability: float = field(
        default=0.15, metadata={"help": "Ratio of spans/sentences to mask for seq2seq language modeling loss"}
    )
    # not needed for our pretraining objective
...
def main():
    ...
    send_example_telemetry("run_t5_seq2seq_lm", model_args, data_args)
    ...
    def tokenize_function(examples):
        ...
        Will need to modify later to fit the pre-training objective:
        currently, it's for multi-target text span prediction (in generation way)
        '''
    ...
    # max_seq_length.
    max_seq_length = min(data_args.max_source_length, tokenizer.model_max_length)
    def group_texts(examples):
        # Concatenate all texts. (NOT NEEDED)
    ...
    # We don't need group_text here because our pre-training objective is GSG instead of MLM.
    ...
    if training_args.do_train:
        ...
        if data_args.max_train_samples is not None:
            max_train_samples = min(len(train_dataset), data_args.max_train_sample)
    ...
    kwargs = {"finetuned_from": model_args.model_name_or_path, "tasks": "gap span/sentence generation"}
    if data_args.dataset_name is not None:
