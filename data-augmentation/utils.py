...
def read_jsonl_inputs(path: str) -> List[Union[Tuple[str, Dict[str, Any]], Tuple[str, str, Dict[str, Any], Dict[str, Any]]]]:
    """Read input texts from a jsonl file, where each line is one json object and input texts are stored in the fields 'text_a' and 'text_b', and metadata is stored in the fields 'meta_a' and 'meta_b'"""
    ds_entries = DatasetEntry.read_list(path)
    print(f"Done loading {len(ds_entries)} inputs from file '{path}'")
    if ds_entries and ds_entries[0].meta_a and ds_entries[0].meta_b is not None:
        return [(entry.text_a, entry.meta_a, entry.meta_b) if entry.text_b is None else (entry.text_a, entry.text_b, entry.meta_a, entry.meta_b) for entry in ds_entries]
    else:
        return [(entry.text_a,) if entry.text_b is None else (entry.text_a, entry.text_b) for entry in ds_entries]
...
class DatasetEntry:
    ...
    def __init__(self, text_a: str, text_b: Optional[str], label: Any, meta_a: Optional[Dict[str, Any]], meta_b: Optional[Dict[str, Any]]):
        self.text_a = text_a
        self.text_b = text_b
        self.label = label
        self.meta_a = meta_a
    ...
    def __repr__(self):
        if self.text_b is not None:
            return f'DatasetEntry(text_a="{self.text_a}", text_b="{self.text_b}", label={self.label}, meta_a={self.meta_a}, meta_b={self.meta_b})'
        else:
            return f'DatasetEntry(text_a="{self.text_a}", label={self.label}, meta_a={self.meta_a}, meta_b={self.meta_b})'

    def __key(self):
        return self.text_a, self.text_b, self.label, self.meta_a, self.meta_b
