...
#os.environ["openai_api_key"] = "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"

PLACEHOLDER_STR = "<X1>"
META_A_PLACEHOLDER_STR = "<META_A>"
META_B_PLACEHOLDER_STR = "<META_B>"
...
class DinoGenerator:
    ...
    def __init__(self, task_spec: Dict[str, Any], model: Union['str', 'ModelWrapper'] = None, openai_api_key: Optional[str] = None,
                 max_output_length: int = 256, decay_constant: float = 100, top_p: float = 0.9, top_k: int = 5,
        ...
        :param allow_newlines_in_outputs: if set to true, model outputs that contain a newline character before the end-of-sequence token
               (a quotation markhttps://github.com/HarrywillDr/MedDEGN) are not removed from the dataset
        """
        self.model = model
    ...
    def generate_dataset(self, input_texts: Optional[List[str]], num_entries_per_input_and_label: Optional[int] = None,
        ...
        for input_text_or_id, meta_a, meta_b in input_iterator:
            if isinstance(input_text_or_id, (str, int)):
                for label in self.labels:
                    dataset += self._generate_dataset_entries(input_text_or_id, label=label, num_entries=num_entries_per_input_and_label,
                                                            generate_with_inputs=generate_with_inputs, meta_a=meta_a, meta_b=meta_b)
            else:
                raise TypeError("input_text_or_id argument must be a string or an integer")
    ...
    """
    def generate_dataset(self, input_texts: Optional[List[str]], num_entries_per_input_and_label: Optional[int] = None,
                        num_entries_per_label: Optional[int] = None, batch_size: Optional[int] = None,
                        save_frequency: Optional[int] = None, save_path: Optional[str] = None) -> List[DatasetEntry]:
        ...
        :param num_entries_per_label: the number of entries to generate for each label
        :param batch_size: the number of entries to generate simultaneously
        :param save_frequency: the frequency with which to save the generated dataset (in number of iterations)
        :param save_path: the path where the generated dataset should be saved
        :return: the generated dataset
        ...
        # Load the previously saved dataset if available
        if save_path is not None and os.path.exists(save_path):
            with open(save_path, 'rb') as f:
                dataset = pickle.load(f)

        for i, (input_text_or_id, meta_a, meta_b) in enumerate(input_iterator):
            if isinstance(input_text_or_id, (str, int)):
                for label in self.labels:
                    dataset += self._generate_dataset_entries(input_text_or_id, label=label, num_entries=num_entries_per_input_and_label,
                                                            generate_with_inputs=generate_with_inputs, meta_a=meta_a, meta_b=meta_b)
            ...
            # Save the generated dataset at regular intervals
            if save_frequency is not None and (i + 1) % save_frequency == 0 and save_path is not None:
                with open(save_path, 'wb') as f:
                    pickle.dump(dataset, f)

        # Save the final generated dataset if a save path is provided
    ...
    def _generate_dataset_entries(self, input_text_or_id: Union[str, int], label: str, num_entries: int, generate_with_inputs: bool,
                            meta_a: Optional[str] = None, meta_b: Optional[str] = None) -> List[DatasetEntry]:
        
        instruction = self._build_instruction(label, input_text_or_id, meta_a=meta_a, meta_b=meta_b, generate_with_inputs=generate_with_inputs)
        chat_instruction = [{"role":"system","content":"You are a helpful healthcare assistant"}]

        if self.openai_api_key is not None:
            try:
                if self.model_is_chat_model:

                    chat_instruction.append({"role":"user","content": instruction})

                    model_responses = [openai.ChatCompletion.create(
                    model=self.model, messages=chat_instruction, max_tokens=self.max_output_length, top_p=self.top_p, stop=['"'],
                    temperature=1) for _ in range(num_entries)]
                else:
                    model_responses = [openai.Completion.create(
                        model=self.model, prompt=instruction, max_tokens=self.max_output_length, top_p=self.top_p, stop=['"']
            ...
            counter_instructions = [
                self._build_instruction(other_label, input_text_or_id, meta_a=meta_a, meta_b=meta_b, generate_with_inputs=generate_with_inputs) for other_label in self.counter_labels[label]
            ]
        ...
        model_outputs = [
            self._process_output(input_text=input_text_or_id, output_text=output, label=label, generate_with_inputs=generate_with_inputs, meta_a=meta_a, meta_b=meta_b)
    ...
    def _build_instruction(self, label: str, text: str, generate_with_inputs: bool, meta_a: Optional[str] = None, meta_b: Optional[str] = None) -> str:
        ...
        if generate_with_inputs:
            ...
            instruction_template = instruction_template.replace(PLACEHOLDER_STR, text)
        else:
        ...
        # add meta_a and meta_b if they are provided and if the instruction template contains the corresponding placeholders
        if meta_a is not None and META_A_PLACEHOLDER_STR in instruction_template:
            instruction_template = instruction_template.replace(META_A_PLACEHOLDER_STR, meta_a)

        if meta_b is not None and META_B_PLACEHOLDER_STR in instruction_template:
            instruction_template = instruction_template.replace(META_B_PLACEHOLDER_STR, meta_b)

        return instruction_template

    def _process_output(self, input_text: Union[str, int], output_text: str, label: str, generate_with_inputs: bool, meta_a: Optional[str] = None, meta_b: Optional[str] = None) \
        ...
        meta_a = meta_a if meta_a else None
        meta_b = meta_b if meta_b else None
        if output_text and ('\n' not in output_text or self.allow_newlines_in_outputs):
            ...
            return DatasetEntry(text_a=text_a, text_b=text_b, meta_a=meta_a, meta_b=meta_b, label=label)
        return None
...
class GPT2Wrapper(ModelWrapper):
    ...
    def __init__(self, model_name: str = "stanford-crfm/BioMedLM", use_cuda: bool = True):
        """
        :param model_name: the name of the pretrained GPT2 model (default: "gpt2-xl")
        :param use_cuda: whether to use CUDA
        """
        super().__init__(use_cuda=use_cuda)
        self._tokenizer = GPT2Tokenizer.from_pretrained(model_name, pad_token="[PAD]")
